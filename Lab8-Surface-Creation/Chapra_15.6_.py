...
def zfun(xe, ye, coefs):
    return (coefs[0]*(xe**0)*ye**0 + coefs[1]*xe**1 + coefs[2]*ye**1)

# %% Reshape data for block matrices
xv = np.reshape(x, (-1, 1))
yv = np.reshape(y, (-1, 1))
zv = np.reshape(z, (-1, 1))
phi_mat = np.block([[(xv**0)*(yv**0), xv**1, yv**1]])
...
fig.tight_layout()
fig.savefig("Chapra_15.6_Plot_Alpha.png")
fig.savefig("Chapra_15.6_Plot_Alpha.eps")
...
fig.tight_layout()
fig.savefig("Chapra_15.6_Plot_Bravo.png")
fig.savefig("Chapra_15.6_Plot_Bravo.eps")
